        ...
        def _insert(data, curNode):
            if curNode.value > data:
                if curNode.left is None:
                    curNode.left = BSTNode(data)
                    return curNode.left
                else:
                    return _insert(data, curNode.left)
            elif curNode.value < data:
                if curNode.right is None:
                    curNode.right = BSTNode(data)
                    return curNode.right
                else:
                    return _insert(data, curNode.right)
            else:
                raise ValueError("value already exists in the node")
...
def prob4():
    ...
    for n in t:
        ...
        rand_find = random.sample(rand, 5)
        start = time.time()
        for data in rand_find:
            linkedlist.iterative_find(data)
        end = time.time()
    ...
    ax1.plot(t, times1, label="List")
    ax1.plot(t, times2, label="BST")
    ax1.plot(t, times3, label="AVL")
    ...
    ax2.semilogy(t, times4, basey=2, label="List")
    ax2.semilogy(t, times5, basey=2, label="BST")
    ax2.semilogy(t, times6, basey=2, label="AVL")
